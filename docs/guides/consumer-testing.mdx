...
def validator():
    ...
    v.register_schema('user-service', './contracts/user-api.json')
    yield v
    v.close()
...
func TestGetUserReturnsValidResponse(t *testing.T) {
    ctx := context.Background()
    validator, err := cvt.NewValidator("localhost:9550")
    assert.NoError(t, err)
    defer validator.Close()

    err = validator.RegisterSchema(ctx, "user-service", "./contracts/user-api.json")
    assert.NoError(t, err)
...
class UserServiceContractTest {
    ...
    static void setup() throws Exception {
        validator = new ContractValidator("localhost:9550");
        validator.registerSchema("user-service", "./contracts/user-api.json");
    }
...
validator.register_schema('user-api', './user-api.json')
...
ctx := context.Background()
validator.RegisterSchema(ctx, "user-api", "./user-api.json")
...
result, _ := validator.Validate(ctx, cvt.ValidationRequest{
    ...
    Body:       map[string]any{"id": "123", "name": "John Doe", "email": "john@example.com"},
})
...
validator.registerSchema("user-api", "./user-api.json");
...
ctx := context.Background()
validator.RegisterSchema(ctx, "user-service", "./contracts/user-api.json")
...
HttpResponse<String> response = client.send(
    HttpRequest.newBuilder().uri(URI.create("http://user-service/users/123")).build(),
...
const mock = createMockAdapter({ validator, schemaId: "user-api", cache: true });

// Make requests - responses are generated from OpenAPI schema
const response = await mock.fetch("http://mock.user-api/users/456");
...
validator = ContractValidator('localhost:9550')
validator.register_schema('user-api', './contracts/user-api.json')

# Create mock session that auto-generates responses from schema
mock = create_mock_session(validator=validator, schema_id='user-api', cache=True)

# Make requests - responses are generated from OpenAPI schema
response = mock.get('http://mock.user-api/users/456')
user = response.json()
...
ctx := context.Background()
validator.RegisterSchema(ctx, "user-api", "./contracts/user-api.json")
...
ContractValidator validator = new ContractValidator("localhost:9550");
validator.registerSchema("user-api", "./contracts/user-api.json");
...
HttpResponse<String> response = mock.send(
    HttpRequest.newBuilder()
        .uri(URI.create("http://mock.user-api/users/456"))
        .build(),
    HttpResponse.BodyHandlers.ofString()
);
...
await mock.fetch("http://mock.user-api/users/123");
await mock.fetch("http://mock.user-api/users", { method: "POST", body: "{}" });
...
const consumerInfo = await validator.registerConsumerFromInteractions(
  mock.getInteractions(),
  {
    ...
    environment: "dev",
    schemaVersion: "1.0.0",
    // schemaId auto-extracted from URL: http://mock.user-api/... -> "user-api"
  },
);
...
# Run tests using the mock
mock.get('http://mock.user-api/users/123')
mock.post('http://mock.user-api/users', json={})
...
consumer_info = validator.register_consumer_from_interactions(
    ...
    # schema_id auto-extracted from URL: http://mock.user-api/... -> 'user-api'
)
...
// Run tests using the mock
mock.get("http://mock.user-api/users/123");
mock.post("http://mock.user-api/users", "{}");
...
- Auto-extracted from mock URLs: `http://mock.user-api/users/123` extracts `user-api`
...
const consumerInfo = await validator.registerConsumerFromInteractions(
  interactions,
  {
    ...
    environment: "dev",
    schemaVersion: "1.0.0",
    schemaId: "user-api", // Explicit override for non-mock URLs
  },
);
```
  ...
  usedEndpoints: [
    {
      method: "GET",
      path: "/users/{id}",
      usedFields: ["id", "name", "email", "address.city", "address.zipCode"],
    },
    {
      method: "POST",
      path: "/users",
...
validator.register_consumer(RegisterConsumerOptions(
    ...
    used_endpoints=[
        EndpointUsage(
            method='GET',
            path='/users/{id}',
            used_fields=['id', 'name', 'email', 'address.city', 'address.zipCode']
        ),
        EndpointUsage(
            method='POST',
            path='/users',
    ...
    UsedEndpoints: []cvt.EndpointUsage{
        {
            Method:     "GET",
            Path:       "/users/{id}",
            UsedFields: []string{"id", "name", "email", "address.city", "address.zipCode"},
        },
        {
            Method:     "POST",
            Path:       "/users",
...
List<EndpointUsage> endpoints = Arrays.asList(
    new EndpointUsage("GET", "/users/{id}",
        Arrays.asList("id", "name", "email", "address.city", "address.zipCode")),
    new EndpointUsage("POST", "/users", Arrays.asList("id"))
);
...
```python
consumers = validator.list_consumers('user-api', 'dev')
print(f"{len(consumers)} services depend on user-api")
```
...
fmt.Printf("%d services depend on user-api\n", len(consumers))
```
...
```java
List<ConsumerInfo> consumers = validator.listConsumers("user-api", "dev");
System.out.printf("%d services depend on user-api%n", consumers.size());
```
...
```python
validator.deregister_consumer('order-service', 'user-api', 'dev')
```
...
```java
validator.deregisterConsumer("order-service", "user-api", "dev");
```
...
cvt can-i-deploy --schema user-api --version 2.0.0 --env staging \
...
endpoints = [
    EndpointUsage(method='GET', path='/users/{id}', used_fields=['id', 'name'])
]
...
endpoints := []cvt.EndpointUsage{
    {Method: "GET", Path: "/users/{id}", UsedFields: []string{"id", "name"}},
}
...
List<EndpointUsage> endpoints = List.of(
    new EndpointUsage("GET", "/users/{id}", List.of("id", "name"))
);
...
```bash
# Before promoting user-api v2.0.0 to production
cvt can-i-deploy --schema user-api --version 2.0.0 --env prod
...
it("catches missing required fields", async () => {
  const result = await validator.validate(
    ...
    {
      statusCode: 200,
      body: { id: "123" }, // missing name, email
    },
  );
...
func TestCatchesMissingRequiredFields(t *testing.T) {
    result, err := validator.Validate(ctx, cvt.ValidationRequest{
        ...
        StatusCode: 200,
        Body:       map[string]any{"id": "123"}, // missing name, email
    })
    assert.NoError(t, err)
...
def test_validates_404_error_response_format(validator):
    result = validator.validate(
        request={'method': 'GET', 'path': '/users/nonexistent'},
        response={
            'status_code': 404,
            'body': {'error': 'User not found', 'code': 'NOT_FOUND'}
        }
...
void validates404ErrorResponseFormat() {
    ValidationResult result = validator.validate(
        ValidationRequest.builder().method("GET").path("/users/nonexistent").build(),
        ValidationResponse.builder()
            .statusCode(404)
            .body("{\"error\": \"User not found\", \"code\": \"NOT_FOUND\"}")
            .build()
