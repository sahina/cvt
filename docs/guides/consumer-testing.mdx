...
See [Breaking Changes Guide](./breaking-changes.mdx) for more details.
...
:::note Docker Image
The published CVT server image is available at `ghcr.io/sahina/cvt-server:latest`. You can also:
...
contract-tests:
  stage: test
  services:
    - name: your-registry/cvt-server:latest
      alias: cvt
  variables:
    CVT_SERVER: "cvt:9550"
  script:
  ...
  script:
    - |
      ...
      if [ "$(jq '.safeToDeploy' check.json)" != "true" ]; then
        ...
        jq '.affectedConsumers[] | select(.willBreak == true)' check.json
        exit 1
      fi
```
        ...
        stage('Can I Deploy') {
            ...
            steps {
                script {
                    ...
                    if (!json.safeToDeploy) {
                        error "Breaking changes detected! Affected consumers: ${json.affectedConsumers}"
                    }
                }
            }
        }
