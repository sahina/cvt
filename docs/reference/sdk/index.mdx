---
title: SDK Reference
sidebar_label: Overview
sidebar_position: 1
description: CVT SDK documentation for all supported languages
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# SDK Reference

## What is the SDK Reference?

This section provides documentation for CVT's language-specific SDKs. Each SDK offers the same core functionality, allowing you to choose the one that best fits your application's technology stack.

:::tip SDK Architecture
For information about SDK design patterns, adapter architecture, and how SDKs maintain cross-language consistency, see [SDK Architecture](../architecture/sdk-architecture.md).
:::

:::tip Published SDKs
CVT SDKs are published to package registries: Node.js to [GitHub Packages](https://github.com/sahina/cvt/packages) and [npmjs](https://www.npmjs.com/package/@sahina/cvt-sdk), Java to [GitHub Packages](https://github.com/sahina/cvt/packages) and [Maven Central](https://central.sonatype.com/artifact/io.github.sahina/cvt-sdk), Python to [PyPI](https://pypi.org/project/cvt-sdk/) and [GitHub Releases](https://github.com/sahina/cvt/releases), and Go via `go get`.
:::

## Available SDKs

| Language | Path | Status |
|----------|------|--------|
| [Node.js](./nodejs.md) | `sdks/node` | Production-ready |
| [Python](./python.md) | `sdks/python` | Production-ready |
| [Go](./go.md) | `sdks/go` | Production-ready |
| [Java](./java.md) | `sdks/java` | Production-ready |

## Installation

<Tabs groupId="sdk-language">
<TabItem value="nodejs" label="Node.js">

```bash
# From npmjs (no auth required)
npm install @sahina/cvt-sdk

# Or from GitHub Packages (recommended — single PAT for all SDKs)
# See Node.js SDK docs for .npmrc setup
```

</TabItem>
<TabItem value="python" label="Python">

```bash
# From PyPI (recommended)
pip install cvt-sdk
```

</TabItem>
<TabItem value="go" label="Go">

```bash
go get github.com/sahina/cvt/sdks/go
```

</TabItem>
<TabItem value="java" label="Java">

```bash
# From GitHub Packages (requires PAT — see Java SDK docs for setup)
# Maven: add github-cvt repository + dependency to pom.xml
# Gradle: add maven repository + dependency to build.gradle
```

</TabItem>
</Tabs>

## Common Features

All SDKs provide:

- **Schema Registration** - Register OpenAPI v2/v3 schemas
- **Interaction Validation** - Validate request/response pairs
- **Consumer Registration** - Register as a consumer for deployment safety
- **Schema Comparison** - Detect breaking changes between versions
- **Fixture Generation** - Generate test data from schemas
- **HTTP Adapters** - Automatic validation for HTTP clients
- **Producer Middleware** - Automatic validation for HTTP servers

## Architecture

```
┌─────────────────────────────────────────────────────────────┐
│                      Your Application                        │
│  ┌───────────┐  ┌───────────┐  ┌───────────┐  ┌───────────┐ │
│  │  Node.js  │  │  Python   │  │    Go     │  │   Java    │ │
│  │    SDK    │  │    SDK    │  │    SDK    │  │    SDK    │ │
│  └─────┬─────┘  └─────┬─────┘  └─────┬─────┘  └─────┬─────┘ │
└────────┼──────────────┼──────────────┼──────────────┼───────┘
         │              │              │              │
         └──────────────┴──────────────┴──────────────┘
                                │
                          gRPC Protocol
                                │
                                ▼
                   ┌────────────────────────┐
                   │      CVT Server        │
                   │     (Port 9550)        │
                   └────────────────────────┘
```

## Quick Comparison

### Initialization

<Tabs groupId="sdk-language">
<TabItem value="nodejs" label="Node.js">

```typescript
import { ContractValidator } from "@sahina/cvt-sdk";

const validator = new ContractValidator("localhost:9550");
```

</TabItem>
<TabItem value="python" label="Python">

```python
from cvt_sdk import ContractValidator

validator = ContractValidator("localhost:9550")
```

</TabItem>
<TabItem value="go" label="Go">

```go
import "github.com/sahina/cvt/sdks/go/cvt"

validator, err := cvt.NewValidator("localhost:9550")
if err != nil {
    log.Fatal(err)
}
defer validator.Close()
```

</TabItem>
<TabItem value="java" label="Java">

```java
import io.github.sahina.sdk.ContractValidator;

ContractValidator validator = new ContractValidator("localhost:9550");
```

</TabItem>
</Tabs>

### Validation

<Tabs groupId="sdk-language">
<TabItem value="nodejs" label="Node.js">

```typescript
const result = await validator.validate(request, response);
if (!result.valid) {
  console.error("Errors:", result.errors);
}
```

</TabItem>
<TabItem value="python" label="Python">

```python
result = validator.validate(request, response)
if not result["valid"]:
    print("Errors:", result["errors"])
```

</TabItem>
<TabItem value="go" label="Go">

```go
result, err := validator.Validate(ctx, request, response)
if err != nil {
    log.Fatal(err)
}
if !result.Valid {
    fmt.Println("Errors:", result.Errors)
}
```

</TabItem>
<TabItem value="java" label="Java">

```java
ValidationResult result = validator.validate(request, response);
if (!result.isValid()) {
    System.err.println("Errors: " + result.getErrors());
}
```

</TabItem>
</Tabs>

## Choosing an SDK

All SDKs have equivalent functionality. Choose based on your application's language.

For detailed documentation, see the individual SDK guides:

- **[Node.js SDK](./nodejs.md)** - For Node.js and TypeScript applications
- **[Python SDK](./python.md)** - For Python applications
- **[Go SDK](./go.md)** - For Go applications
- **[Java SDK](./java.md)** - For Java and JVM applications
