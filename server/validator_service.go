...
func (s *ValidatorService) CanIDeploy(ctx context.Context, req *pb.CanIDeployRequest) (*pb.CanIDeployResponse, error) {
	...
	// For now, check if any consumers are on a different schema version
	// In a full implementation, we would:
	// 1. Compare the new version with each consumer's tested version
	// 2. Detect breaking changes
	// 3. Filter changes to those affecting each consumer's used endpoints
	var affectedConsumers []*pb.ConsumerImpact
	allSafe := true

	for _, consumer := range consumers {
		// If consumer is on a different version, mark as potentially affected
		willBreak := consumer.SchemaVersion != "" && consumer.SchemaVersion != req.NewVersion

		if willBreak {
			allSafe = false
		}

		protoEndpoints := make([]*pb.EndpointUsage, len(consumer.UsedEndpoints))
		for i, eu := range consumer.UsedEndpoints {
			protoEndpoints[i] = &pb.EndpointUsage{
				Method:     eu.Method,
				Path:       eu.Path,
				UsedFields: eu.UsedFields,
	...
	if allSafe {
		...
		summary = fmt.Sprintf("Deployment of %s version %s to %s may affect %d consumer(s) - review required",
			req.SchemaId, req.NewVersion, environment, len(affectedConsumers))
	}

	// TODO: In a full implementation, we would compare schemas here for breaking changes
	var breakingChanges []*pb.BreakingChange
